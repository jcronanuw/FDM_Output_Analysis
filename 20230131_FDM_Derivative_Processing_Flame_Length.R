#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#PURPOSE: This script analyzes patterns in simulated flame length potentials for
...
f.head <- raster("w050_01_00.asc")
maps_orig <- matrix(scan("w050_01_00.asc",skip = f.head@file@offset),ncol=f.head@ncols,byrow=T)
#50K/YEAR RUN
