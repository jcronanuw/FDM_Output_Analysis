#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#PURPOSE: This script generates tabular data for the FCCS fuelbed vegetation cover categoriesc 
#from FDM FCCS secondary maps (primary are the fuelbed maps, seconday are the fuelbed
#derivative maps)
...
f.head <- raster("r050_01_00.asc")
maps_orig <- matrix(scan("r050_01_00.asc",skip = f.head@file@offset),ncol=f.head@ncols,byrow=T)
#50K/YEAR RUN
...
print(paste("Min crown fire potential: ", d_min))
print(paste("Max crown fire potential: ", d_max))
