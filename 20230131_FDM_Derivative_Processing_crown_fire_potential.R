#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#PURPOSE: This script analyzes patterns in simulated crown fire potential for
...
f.head <- raster("z050_01_00.asc")
maps_orig <- matrix(scan("z050_01_00.asc",skip = f.head@file@offset),ncol=f.head@ncols,byrow=T)
#50K/YEAR RUN
...
#Input file pre-fix
prefix_in <- "z"
prefix_name <- "crown_fire_potential"
  ...
  for(b in 1:length(run_number))
  {
    ...
    derivative_matrix <- matrix(nrow = length(maps_in), ncol = d_limit)
      ...
      for(n in 1:length(maps_in))
        {
        for(o in 1:(d_limit*2))
        {
          derivative_matrix[n,o] <- length(maps_in[[n]][unit_map != 8888  & !fuelbed_map %in% c(5099000, 6000000) & maps_in[[n]] > ((o/2)-10) & maps_in[[n]] <= (o/2)])
        }
      }
