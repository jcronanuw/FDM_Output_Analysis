#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#PURPOSE: This script analyzes patterns in simulated fuel and fire behavior potentials for
...
#################################################################################################
setwd(paste("D:/FDM_2023_Simulation_Data/Step_02_Fuelbed_Derivative_Maps/maps", sep = ""))
...
setwd(paste("C:/Users/jcronan/OneDrive - USDA/Documents/GitHub/EglinAirForceBase/inputs", sep = ""))
unit_map <- matrix(scan("sef_bmap_1771x3491.txt", skip = f.head@file@offset),ncol=f.head@ncols,byrow=T)

setwd(paste("D:/FDM_2023_Simulation_Data/Step_01_FDM_Outputs/fuelbed_maps_step_1b_short_file_names", sep = ""))
...
for(a in 1:length(rx_fire))
{
  for(b in 1:length(run_number))
  {
    ...
    for(i in 1:length(maps_in))
      {
      max_load[i] <- max(as.vector(maps_in[[i]][unit_map != 8888]))
      }
    ...
    fineFuels_matrix <- matrix(nrow = length(maps_in), ncol = ff_limit)
      ...
      for(n in 1:length(maps_in))
        ...
        for(o in 1:ff_limit)
        {
          fineFuels_matrix[n,o] <- length(maps_in[[n]][unit_map != 8888  & !fuelbed_map %in% c(5099000, 6000000) & maps_in[[n]] > (o-1) & maps_in[[n]] <= o])
        }
      }
...
setwd("D:/FDM_2023_Simulation_Data/Step_05_Derivative_Tables")
